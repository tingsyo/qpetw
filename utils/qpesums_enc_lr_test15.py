...
def main():
    ...
    for sid in stdids:
        ...
        idx2015 = np.floor(stdio['date'].astype(float)/1000000.) == 2015
        size_of_2015 = sum(idx2015)
        size_before_2015 = sum(stdio['date'].astype(int)<2015010100)
        logging.info('    Data index of 2015: '+str(size_of_2015))
        #y_train = y.iloc[:size_before_2016]
        #x_train = x.iloc[:size_before_2016,:]
        y_train = y.loc[~idx2015]
        x_train = x.loc[~idx2015,:]
        ...
        y_test = y.loc[idx2015].reset_index(drop=True)
        x_test = x.loc[idx2015,:].reset_index(drop=True)

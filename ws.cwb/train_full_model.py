...
def main():
    ...
    # Output full model
    model[0].save(args.output+'_mlc.h5')
    with open(args.output+'_mlc_model.json', 'w') as outfile:
        outfile.write(model[0].to_json())
    model[0].save_weights(args.output+'_mlc_weights.h5')
    # Output encoder
    model[1].save(args.output+'_encoder.h5')
    with open(args.output+'_encoder_model.json', 'w') as outfile:
        outfile.write(model[1].to_json())
    model[1].save_weights(args.output+'_encoder_weights.h5')
    # done
    return(0)
